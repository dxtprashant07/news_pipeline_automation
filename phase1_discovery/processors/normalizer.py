...
from ..utils.base import RawStory
from ..utils.helpers import url_hash, content_hash, normalize_url, truncate, domain_from_url, utcnow
...
class NormalizedStory:
    ...
    This is the object that flows through all downstream processors.
    """
    # Identity
    url_hash: str
    content_hash: str
    ...
    # Pass-through metadata from the source
    extra: dict = field(default_factory=dict)


class Normalizer:
    """Converts RawStory objects into NormalizedStory objects."""

    def normalize(self, raw: RawStory) -> NormalizedStory | None:
        """Returns None if the story is unusable (no title or URL)."""
    ...
    def normalize_all(self, raws: list[RawStory]) -> list[NormalizedStory]:
        """Normalize a batch, silently dropping any invalid stories."""
        results = []
        for raw in raws:
            try:
