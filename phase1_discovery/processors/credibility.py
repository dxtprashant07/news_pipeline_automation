...
# Curated domain trust scores (0-100).
# Add/adjust as you expand coverage.
DOMAIN_SCORES: dict[str, int] = {
    ...
    "reddit.com": 40,   # Aggregator — low by default
    # Tier 3 — unknown (default 50, handled below)
}
...
class CredibilityScorer:
    def score(self, story: NormalizedStory) -> NormalizedStory:
        ...
        # Freshness bonus — stories under 6 hours get +5
        from ..utils.helpers import utcnow
        age_hours = (utcnow() - story.published_at).total_seconds() / 3600
        if age_hours < 6:
            base += 5
        elif age_hours > 48:
            base -= 10  # Old stories lose relevance

        # Has image & summary — small quality signal
        if story.image_url:
            base += 2
