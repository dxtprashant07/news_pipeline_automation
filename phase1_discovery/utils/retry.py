...
class CircuitBreaker:
    """
    Tracks failures per source. After `failure_threshold` consecutive
    failures the circuit opens and blocks calls for `reset_timeout` seconds.
    """
    name: str
...
# Global registry so each source has its own breaker
...
def with_retry(
    ...
    """
    Decorator — retries with exponential backoff + optional circuit breaker.

    Usage:
        @with_retry(max_attempts=3, circuit_name="newsapi")
        def fetch():
            ...
    """
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if breaker.is_open():
                raise RuntimeError(
                    f"Circuit [{breaker.name}] is OPEN — skipping call to protect API limits."
                )
